...
import { DEFAULT_ENUM_INDEX, SHORT_WILDCARD } from '../../src/wildcard';
describe('constraints/spec', () => {
    ...
    describe('autoAddCount', () => {
        function autoCountShouldBe(expectedAutoCount, when, baseSpecQ) {
            [true, false].forEach((satisfy) => {
                ...
                it(`should return ${satisfy} when autoCount is ${autoCount} and ${when}.`, () => {
                    ...
                    const model = SpecQueryModel.build(specQ, schema, Object.assign(Object.assign({}, DEFAULT_QUERY_CONFIG), { autoAddCount: true }));
                    const autoCountIndex = model.getEncodings().length - 1;
                    model.setEncodingProperty(autoCountIndex, 'autoCount', autoCount, model.getEncodingQueryByIndex(autoCountIndex).autoCount);
    ...
    describe('hasAppropriateGraphicTypeForMark', () => {
        ...
        describe('line, area', () => {
            ...
            it('should return true for aggregate line/area with one temporal field', () => {
                [MARK.LINE, MARK.AREA].forEach((mark) => {
                    [undefined, 'year'].forEach((timeUnit) => {
                        const specM = buildSpecQueryModel({
                            mark: mark,
                            encodings: [
        ...
        it('should return false for stack with non linear scale type', () => {
            [
                ...
                NON_NULL_STACK_OFFSETS.forEach((stackOffset) => {
                    ...
                    assert.isFalse(SPEC_CONSTRAINT_INDEX['omitInvalidStackSpec'].satisfy(specM, schema, DEFAULT_QUERY_CONFIG), `for ${scaleType}`);
                });
            });
        });
