...
import { isAutoCountQuery } from '../src/query/encoding';
...
describe('generate', function () {
    ...
    describe('2D', () => {
        ...
        describe('NxO', () => {
            ...
            it('should generate a rect table with x and y as dimensions with autocount turned off', () => {
                answerSet.forEach((specM) => {
                    assert.isTrue(specM.getEncodingQueryByChannel(CHANNEL.X).type === TYPE.NOMINAL &&
    ...
    describe('3D', () => {
        describe('NxNxQ', () => {
            ...
            it('should not generate a plot with both x and y as dimensions with auto add count enabled', () => {
                answerSet.forEach((specM) => {
                    assert.isFalse(specM.getEncodingQueryByChannel(CHANNEL.X).type === TYPE.NOMINAL &&
        ...
        describe('NxNxQ with x = N1 and y = N2', () => {
            ...
            it('should generate a rect heatmap with color encoding the quantitative measure', () => {
                answerSet.forEach((specM) => {
                    assert.isTrue(specM.getMark() === MARK.RECT &&
    ...
    describe('axis-zindex', () => {
        it('should enumerate default axisLayers', () => {
            ...
            const answerSet = generate(specQ, schema);
            assert.equal(answerSet.length, 2);
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).axis.zindex, 1);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).axis.zindex, 0);
        });
    });
    describe('scale-rangeStep', () => {
        it('should enumerate correct scaleType with width step', () => {
            ...
            const answerSet = generate(specQ, schema);
            assert.equal(answerSet.length, 2);
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, undefined);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).scale.type, ScaleType.POINT);
        });
    });
    describe('scale-exponent', () => {
        it('should enumerate correct scale type when scale clamp is used with scale exponent and TYPE.Quantitative', () => {
            ...
            const answerSet = generate(specQ, schema);
            assert.equal(answerSet.length, 1);
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, ScaleType.POW);
        });
    });
    describe('scale-nice', () => {
        it('should enumerate correct scale type when scale nice is used with scale round and TYPE.QUANTITATIVE', () => {
            ...
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, undefined);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).scale.type, ScaleType.LINEAR);
            assert.equal(answerSet[2].getEncodingQueryByIndex(0).scale.type, ScaleType.LOG);
            assert.equal(answerSet[3].getEncodingQueryByIndex(0).scale.type, ScaleType.POW);
            assert.equal(answerSet[4].getEncodingQueryByIndex(0).scale.type, ScaleType.SQRT);
        });
    });
    describe('scale-zero', () => {
        it('should enumerate correct scale type when scale zero is used without bar mark or binning', () => {
            ...
            assert.equal(answerSet.length, 2);
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, undefined);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).scale.type, ScaleType.SQRT);
        });
    ...
    describe('scale-type', () => {
        it('should enumerate correct scale enabling and scale type for quantitative field', () => {
            ...
            assert.equal(answerSet.length, 3);
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, undefined);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).scale.type, ScaleType.LOG);
        ...
        it('should enumerate correct scale type for temporal field without timeunit', () => {
            ...
            const answerSet = generate(specQ, schema);
            assert.equal(answerSet.length, 3);
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, ScaleType.TIME);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).scale.type, ScaleType.UTC);
            assert.equal(answerSet[2].getEncodingQueryByIndex(0).scale.type, undefined);
        });
        it('should enumerate correct scale type for temporal field with timeunit', () => {
            ...
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, ScaleType.TIME);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).scale.type, ScaleType.UTC);
            assert.equal(answerSet[2].getEncodingQueryByIndex(0).scale.type, ScaleType.POINT);
            assert.equal(answerSet[3].getEncodingQueryByIndex(0).scale.type, undefined);
        });
        it('should enumerate correct scale type for ordinal field with timeunit', () => {
            ...
            const answerSet = generate(specQ, schema);
            assert.equal(answerSet.length, 2);
            assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.type, ScaleType.POINT);
            assert.equal(answerSet[1].getEncodingQueryByIndex(0).scale.type, undefined);
        });
    ...
    describe('stylizer', () => {
        it('should generate answerSet when all stylizers are turned off', () => {
            ...
            const CONFIG_WITHOUT_HIGH_CARDINALITY_OR_FACET = extend({}, DEFAULT_QUERY_CONFIG, { nominalColorScaleForHighCardinality: null }, { smallRangeStepForHighCardinalityOrFacet: null });
        ...
        describe('nominalColorScaleForHighCardinality', () => {
            it('should output range = category20', () => {
                ...
                assert.equal(answerSet[0].getEncodingQueryByIndex(0).scale.scheme, 'category20');
            });
        });
        describe('smallRangeStepForHighCardinalityOrFacet', () => {
            it('should output height step = 12', () => {
                ...
                const answerSet = generate(specQ, schema, DEFAULT_QUERY_CONFIG);
                assert.equal(answerSet[0].specQuery.height.step, 12);
            });
            it('should output height step = 12', () => {
                const specQ = {
                    mark: MARK.BAR,
