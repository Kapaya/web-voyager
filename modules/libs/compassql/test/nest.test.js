...
import { REPLACE_BLANK_FIELDS, REPLACE_FACET_CHANNELS, REPLACE_MARK_STYLE_CHANNELS, REPLACE_XY_CHANNELS, } from '../src/query/groupby';
...
describe('nest', () => {
    ...
    describe('field, aggregate, bin, timeUnit, channel, type', () => {
        ...
        it('should separate different types of stacked and non-stacked visualizations', () => {
            ...
            const groups = nest(answerSet, query.nest).items;
            assert.equal(groups.length, 2);
            assert.equal(groups[0].name, 'style:N1,n|xy:N,n|xy:sum(Q,q)');
            groups[0].items.forEach((item) => {
                return !contains([MARK.BAR, MARK.AREA], item.getMark());
            });
            assert.equal(groups[1].name, 'style:N1,n|xy:N,n|xy:sum(Q,q,stack="zero")');
        ...
        it('should separate different types of stacked and non-stacked visualizations even if it is nested', () => {
            ...
            assert.equal(groups[0].name, 'style:N1,n|xy:N,n|xy:sum(Q,q)');
            groups[0].items.forEach((item) => {
                return !contains([MARK.BAR, MARK.AREA], item.items[0].getMark());
            });
            assert.equal(groups[1].name, 'style:N1,n|xy:N,n|xy:sum(Q,q,stack="zero")');
            groups[1].items.forEach((item) => {
                return contains([MARK.BAR, MARK.AREA], item.items[0].getMark());
    ...
    describe('fieldTransform, encoding', () => {
        it('should group visualization with same fields and transformations, then by encoding', () => {
            ...
            assert.equal(groups[0].items[0].name, 'xy:O,o|xy:mean(Q,q)');
            assert.equal(groups[1].items.length, 1);
            assert.equal(groups[1].items[0].name, 'xy:O,o|xy:median(Q,q)');
        });
    });
});
