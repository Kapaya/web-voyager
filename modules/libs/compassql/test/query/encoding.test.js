...
import { LOCAL_SINGLE_TIMEUNIT_INDEX, UTC_SINGLE_TIMEUNIT_INDEX, LOCAL_MULTI_TIMEUNIT_INDEX, UTC_MULTI_TIMEUNIT_INDEX, } from 'vega-lite/build/src/timeunit';
...
describe('query/encoding', () => {
    describe('toFieldDef', () => {
        ...
        it('return correct fieldDef for FieldQuery', () => {
            assert.deepEqual(toFieldDef({ bin: false, channel: 'x', field: 'Q', type: 'quantitative' }, { props: ['bin', 'timeUnit', 'type'], wildcardMode: 'skip' }), { type: 'quantitative' });
        });
        it('return correct fieldDef for Text FieldQuery with format', () => {
            assert.deepEqual(toFieldDef({ format: '.3f', channel: 'text', field: 'Q', type: 'quantitative' }, { props: ['field', 'format', 'type'], wildcardMode: 'skip' }), { format: '.3f', field: 'Q', type: 'quantitative' });
        });
    });
    describe('toValueDef', () => {
